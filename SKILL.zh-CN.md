...
升级后重新跑体检和离线自检，不要在未确认兼容性时用新旧 CLI 混跑。
...
- **回测区间**，以服务端实测上限为准；CLI 0.1.4 虽展示 10 年能力，但 2026-08-05 实测超过 3 年被拒，同时约定一段不重叠区间留作样本外验证。
- **本次预算**，这一轮允许花多少次运行。
...
| 约束 | 取值 |
|---|---|
| 回测区间 | CLI 0.1.4 宣称最长 10 年；2026-08-05 服务端实测超过 3 年被拒，使用前必须短区间探测 |
...
| `scripts/selftest.py` | 上面三个脚本的离线自检，改动它们之后跑一遍 |
